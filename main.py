...
def main():
    ...
    print(f"Квадратична модель: y = {quad_intercept:.3f} + {quad_coef[0]:.3f} * v + {quad_coef[1]:.3f} * v^2")
    print(f"RANSAC модель: y = {ransac_intercept:.3f} + {ransac_coef[0]:.3f} * v + {ransac_coef[1]:.3f} * v^2\n")
    
    # Step 4: Calculate metrics
    print("----------- ЕТАП 3: ПОРІВНЯННЯ МЕТРИК -----------")
    ...
    print(f"Бутстреп завершено.")
    print(f"95% довірчий інтервал для v_opt: [{ci_lower:.2f}, {ci_upper:.2f}] км/год\n")
    
    # Step 7: RANSAC results
    print("----------- ЕТАП 6: РОБАСТНА РЕГРЕСІЯ (RANSAC) -----------")
    print(f"RANSAC ідентифікував {outlier_info['n_outliers']} викидів.\n")
    
    # Step 8: Visualization
    print("----------- ЕТАП 7: ВІЗУАЛІЗАЦІЯ -----------")
    plot_data = {
        'X': X.ravel(),
        'y': y,
    ...
    # Step 9: Save results
    print("----------- ЕТАП 8: ЗБЕРЕЖЕННЯ РЕЗУЛЬТАТІВ -----------")
    FileHandler.save_summary(quadratic_model, v_opt, y_opt, ci_lower, ci_upper, v_opt_ransac)
    FileHandler.save_metrics(metrics_df)
    
    print(f"Підсумкові коефіцієнти збережено у: team(2)_summary.csv")
    print(f"Підсумкові метрики збережено у: team(2)_metrics.csv\n")
